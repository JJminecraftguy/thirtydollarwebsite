...
const actions = [
    ...
    { shortcut: "x", action: "Stop all sounds", name: "cut", soundTarget: true, stopSounds: true, image: "assets/action_cut.png" },
...
fetch("./sounds.json").then(x => x.json()).then(list => {
    ...
    list.forEach(x => {
        x.imageLink = (!x.emoji && x.id.match(/[a-z0-9]/i)) ? `icons/${x.img || x.id}.png` : `https://cdnjs.cloudflare.com/ajax/libs/twemoji/14.0.2/svg/${(x.emoji || x.id).codePointAt(0).toString(16)}.svg`
        $('#icons').append(`<div class="sound" soundName="${x.name}" soundOrigin="${x.source || ""}" sound="sounds/${x.id}.wav" str="${x.id}" ${(x.tags || []).map(x => "tag_" + x).join(" ")}><img alt="${x.name}" src="${x.imageLink}"></div>`)
        lastGroup = x.group
    })
...
function getSound(id) {
    return soundList.find(x => x.id != "_pause" && (x.id == id || x.emoji == id))
}
...
$(document).on('click contextmenu', '.iconbox, #proHotbar, .soundObj', function () { return false })

$(document).on('contextmenu', '.soundObj', function() { $(this).trigger('click'); return false; })
...
$(document).on('click contextmenu', '#actions div, #hotbarNotes div.action', function (event) {
    ...
    if (action.soundTarget) {
        if (soundTargetFilter) addSoundTarget(added, soundTargetFilter)
    }
    if (action.stopSounds) stopSounds(soundTargetFilter)
...
function addSoundTarget(element, sound) {
    element.append(`<span class="soundBadge" style='background-image: url("${sound.imageLink}")'></span>`)
    element.attr("soundtarget", sound.id)
}

function clearSoundTarget(element) {
    element.find(".soundBadge").remove()
    element.removeAttr("soundtarget")
}
...
function getAudioParams(el, stopPrevious) {
    return { pitch: getPitch(el), volume: getVolume(el), pan: getPan(el), stopPrevious: !!stopPrevious }
}
...
$(document).on('click', '#sequence div', function () {
    ...
    if (shiftHeld || altHeld) {
        ...
        playSound($(this).attr("sound"), getAudioParams($(this), true))
    }
    else $(this).remove()
...
$(document).on('contextmenu', '#sequence div', function () {
    ...
    if (shiftHeld || altHeld) { // clone and append to end
        ...
        playSound(snd, getAudioParams($(this), true))
        return false
    }
        ...
        else if (foundAction.soundTarget) return clearSoundTarget($(this))
        else if (!foundAction.amount) return false
        replaceAction = $(this)
...
function clampPitch(val) { return Number(clamp(val, -60, 60).toFixed(2)) }
function clampVolume(val) { return Number(clamp(val, 0, 400).toFixed(2)) }
function clampPan(val) { return Math.round(clamp(val, -100, 100)) }
...
$(document).on('wheel touchmove', '#sequence div', function(event) {
    ...
    let isCtrl = (ctrlHeld && el.attr("sound"))
    let xScroll = event.arrowDeltaX || (event.originalEvent ? event.originalEvent.deltaX : 0) || 0
    if (xScroll <= 250 && xScroll >= -250) xScroll = 0 // requires super strong x scroll to prevent accidents
    let yScroll = event.arrowDelta || (event.originalEvent ? event.originalEvent.deltaY : 0) || 0
    if (!xScroll && !yScroll) return
    let downward = (yScroll || -xScroll) > 0
    ...
    if (el.attr("sound") && !el.attr("str").startsWith("_")) {
        let shiftChange = ((downward ? -1 : 1) * (shiftHeld ? (isCtrl ? 10 : 6) : altHeld ? 0.2 : 1))

        // Pitch
        if (!isCtrl) {
            shift = clampPitch(shift + shiftChange)
            let prefix = shift > 0 ? "+" : ""
            if (foundText && shift == 0) el.find("p").remove()
            else if (!el.find("p").length) el.append(`<p>${prefix + shift}</p>`)
            else el.find("p").text(prefix + shift)
            el.attr("pitch", shift)
            playSound(el.attr("sound"), { pitch: semitonesToPercent(shift), volume: getVolume(el), pan: getPan(el), stopPrevious: true })
        }

        // Pan
        else if (!yScroll && xScroll) {
            shift = Number(el.attr("pan")) || 0
            shiftChange *= 5
            shift = clampPan(shift + shiftChange)
            let panStr = `${shift < 0 ? "◂" : ""}${Math.abs(Math.round(shift) / 10)}${shift > 0 ? "▸" : ""}`
            if (shift == 0) el.find("pan").remove()
            else if (!el.find("pan").length) el.append(`<pan>${panStr}</pan>`)
            else el.find("pan").text(panStr)
            el.attr("pan", shift)
            playSound(el.attr("sound"), { pitch: getPitch(el), volume: getVolume(el), pan: shift, stopPrevious: true })
        }
        ...
        // Volume
            ...
            shift = clampVolume(shift + shiftChange)
            if (shift == 100) el.find("vol").remove()
            else if (!el.find("vol").length) el.append(`<vol>${shift}%</vol>`)
            else el.find("vol").text(shift + "%")
            el.attr("vol", shift)
            playSound(el.attr("sound"), { pitch: getPitch(el), volume: shift / 100 / 2, pan: getPan(el), stopPrevious: true })
        }
        setUnsavedChanges(true)
    }
...
// prevent ctrl+zoom
$('.iconbox, #proHotbar, #proTips').bind('mousewheel DOMMouseScroll', function(e) { if (e.ctrlKey) e.preventDefault(); });
...
function getVolume(element) { return Number(element.attr('vol') || 100) / 200 }
function getPan(element) { return Number(element.attr('pan') || 0) }
...
function updateTranspose(trans) {
    $('#transrights').toggle(trans != 0)
    $('#trans').text(`${trans >= 0 ? "+" : ""}${+trans.toFixed(4)}`)
}
...
// stop all sounds
function stopSounds(filter) {
    activeSounds.forEach(x => {
        if (!filter || x.name == `sounds/${filter.id}.wav`) killSound(x.sound, x.gainNode, false)
    })
...
async function playSound(name, soundSettings={}) {
    ...
    let panVal = +soundSettings.pan || 0
    if (panVal) {
        if (settings.invertPanning) panVal *= -1
        let pan = new StereoPannerNode(soundcloud, { pan: panVal / 100 } )
        snd.connect(pan)
        pan.connect(vol)
    }
    else snd.connect(vol)
...
function cutSounds(time, filter) {
    activeSounds.forEach(x => {
        if (!x.cut && x.start <= time && (filter ? x.name == `sounds/${filter}.wav` : true)) {
            x.cut = true;
            x.sound.stop(time);
        }
    })
...
$(document).on('keydown', function(e) {
    let repeat = e.originalEvent.repeat;
    if (e.target.nodeName == "INPUT" || mobile) return
    let popupVisible = $('.popup').is(":visible")
    let hover = $('#sequence div:hover');

    if (e.which == 191 && !repeat) { // slash key (toggle menus)
        if (shiftHeld && !$('#shortcutMenu').is(":visible")) $('#settingsMenu').toggle()
        else if (ctrlHeld && !$('#settingsMenu').is(":visible")) $('#shortcutMenu').toggle()
    }

    else if (!popupVisible) {
        if ([13, 32].includes(e.which) && !repeat) { // enter, space
            ...
            if (hover.length) {
                hover.first().trigger({type: "wheel", arrowDelta: e.which == 40 ? 69 : -69})
                e.preventDefault()
            }
        ...
        else if (ctrlHeld && [37, 39].includes(e.which)) {  // left, right
            if (hover.length) {
                hover.first().trigger({type: "wheel", arrowDeltaX: e.which == 37 ? -500 : 500})
                e.preventDefault()
            }
        }
        ...
        else if (e.which == 88 && hover.length) {  // x
            soundTargetFilter = getSound(hover.attr("str"))
            if (soundTargetFilter) {
                $(`#actions .action[action="cut"]`).trigger("click")
                soundTargetFilter = null;
            }
        }
...
$(document).on('keyup keydown click contextmenu wheel touchmove', updateKeys)
